...
class LPIPSWithDiscriminator(nn.Module):
    ...
    def forward(
        ...
        # print('loss shape: ', inputs.shape, reconstructions.shape)
        # exit()
...
### Modified for 1dcnn lpips -> mse


class MSEWithDiscriminator(nn.Module):
    def __init__(
        self,
        disc_start,
        logvar_init=0.0,
...
class NLayerDiscriminator3D(nn.Module):
    ...
    def forward(self, input):
        """Standard forward."""
        print('discrim', input.dtype, self.main[0].weight.dtype)
        
        # target_dtype = self.main[0].weight.dtype
        # if input.dtype != target_type:
...
class LPIPSWithDiscriminator3D(nn.Module):
    ...
    def forward(
        ...
        #     log = {
        #         "{}/total_loss".format(split): loss.clone().detach().mean(),
        #         "{}/logvar".format(split): self.logvar.clone().detach(),  # Added .clone()
        #         "{}/kl_loss".format(split): kl_loss.detach().mean(),
        #         "{}/nll_loss".format(split): nll_loss.detach().mean(),
        #         "{}/rec_loss".format(split): rec_loss.detach().mean(),
        #         "{}/d_weight".format(split): d_weight.detach(),
